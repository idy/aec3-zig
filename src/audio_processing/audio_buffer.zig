...
pub const AudioBuffer = struct {
    ...
    pub fn num_channels(self: AudioBuffer) usize {
        return self.num_channels_;
    }

    pub fn num_frames(self: AudioBuffer) usize {
        return self.buffer_num_frames;
    }

    pub fn num_frames_per_band(self: AudioBuffer) usize {
        return self.num_split_frames;
    }

    pub fn num_bands(self: AudioBuffer) usize {
        return self.num_bands_;
    }

    pub fn channel(self: *const AudioBuffer, channel_idx: usize) []const f32 {
        return self.data.channel(channel_idx);
    }

    pub fn channel_mut(self: *AudioBuffer, channel_idx: usize) []f32 {
        return self.data.channel_mut(channel_idx);
    }

    pub fn split_band(self: *const AudioBuffer, channel_idx: usize, band_idx: usize) []const f32 {
        if (self.split_data) |*split| {
    ...
    pub fn split_band_mut(self: *AudioBuffer, channel_idx: usize, band_idx: usize) []f32 {
    ...
    pub fn split_into_frequency_bands(self: *AudioBuffer) void {
        if (self.num_bands_ > 1) {
            self.splitting_filter.?.analysis(&self.data, &self.split_data.?);
        }
    }

    pub fn merge_frequency_bands(self: *AudioBuffer) void {
        if (self.num_bands_ > 1) {
...
test "audio_buffer from_sample_rates creates correct buffer" {
    ...
    var buffer = try AudioBuffer.from_sample_rates(arena.allocator(), 16000, 2, 16000, 2, 16000);
    defer buffer.deinit();
...
test "audio_buffer set_num_channels reduces channel count" {
    ...
    var buffer = try AudioBuffer.new(arena.allocator(), 160, 4, 160, 4, 160);
    defer buffer.deinit();
...
test "audio_buffer boundary rejects invalid channel setup" {
    ...
    try std.testing.expectError(error.InvalidBufferChannelCount, AudioBuffer.new(arena.allocator(), 160, 1, 160, 2, 160));
}
...
test "audio_buffer boundary rejects zero frames" {
    ...
    try std.testing.expectError(error.InvalidFrameCount, AudioBuffer.new(arena.allocator(), 0, 1, 160, 1, 160));
    try std.testing.expectError(error.InvalidFrameCount, AudioBuffer.new(arena.allocator(), 160, 1, 0, 1, 160));
    try std.testing.expectError(error.InvalidFrameCount, AudioBuffer.new(arena.allocator(), 160, 1, 160, 1, 0));
}
...
test "audio_buffer boundary rejects zero channels" {
    ...
    try std.testing.expectError(error.InvalidChannelCount, AudioBuffer.new(arena.allocator(), 160, 0, 160, 1, 160));
    try std.testing.expectError(error.InvalidChannelCount, AudioBuffer.new(arena.allocator(), 160, 1, 160, 0, 160));
}
...
test "audio_buffer num_bands returns correct value" {
    ...
    var buffer_48k = try AudioBuffer.new(arena.allocator(), 480, 1, 480, 1, 480);
    defer buffer_48k.deinit();
...
test "audio_buffer new cleans up on allocation failure" {
    ...
    try std.testing.expectError(error.OutOfMemory, AudioBuffer.new(alloc, 480, 1, 480, 1, 480));
