    ...
    pub fn insert_block(self: *Self, block: [][][]f32) void {
        ...
        for (0..self.num_bands) |b| {
            ...
            for (0..self.num_channels) |c| {
                ...
                std.debug.assert(self.buffered_len[b][c] == 0);
    ...
    pub fn insert_block_and_extract_sub_frame(
        ...
        for (0..self.num_bands) |b| {
            ...
            for (0..self.num_channels) |c| {
                ...
                @memcpy(sub_frame[b][c][0..buffered], self.buffer[b][c][0..buffered]);
...
test "block_framer extracts subframe with overlap" {
