...
pub const BlockDelayBuffer = struct {
    ...
    pub fn delay_signal(self: *Self, frame: *frame_audio_buffer.FrameAudioBuffer) !void {
...
test "block_delay_buffer zero delay passthrough" {
    var ab = try frame_audio_buffer.FrameAudioBuffer.from_sample_rates(std.testing.allocator, 16_000, 1, 16_000, 1, 16_000);
    defer ab.deinit();
...
test "block_delay_buffer delays all channels and bands" {
    var ab = try frame_audio_buffer.FrameAudioBuffer.from_sample_rates(std.testing.allocator, 32_000, 2, 32_000, 2, 32_000);
    defer ab.deinit();
