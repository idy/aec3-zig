...
test "golden_num_bands_for_rate" {
    const rates = common.parseNamedI32(golden_text, "NUM_BANDS_RATES", 4);
    const expected = common.parseNamedUsize(golden_text, "NUM_BANDS_EXPECTED", 4);
...
test "golden_fast_approx_log2f" {
    const inputs = common.parseNamedF32(golden_text, "FAST_LOG2_INPUT", 1000);
    const expected = common.parseNamedF32(golden_text, "FAST_LOG2_EXPECTED", 1000);
    for (inputs, expected) |x, exp| {
        const actual = aec3.Aec3Common.fast_approx_log2f(x);
        try common.expectUlpEq(exp, actual, 1);
    }
}

test "golden_fft_data_spectrum_0" {
    const packed_array = common.parseNamedF32(golden_text, "SPECTRUM_PACKED_0", aec3.Aec3Common.FFT_LENGTH);
    const expected = common.parseNamedF32(golden_text, "SPECTRUM_EXPECTED_0", aec3.Aec3Common.FFT_LENGTH_BY_2_PLUS_1);
    ...
    for (expected, out) |e, a| {
        try common.expectUlpEq(e, a, 1);
    }
}

test "golden_config_defaults" {
    const expected = common.parseNamedF64(golden_text, "CONFIG_DEFAULTS", 20);
